...
/// Vote-lifecycle scheduled jobs (activate/close/remind) extracted into their
/// own module to keep this file focused on the core tick loop. Like
/// [`retention`], the submodule attaches its jobs to `Scheduler` via a
/// dedicated `impl Scheduler` block; behaviour is identical to the previous
