...
// # RLS (PAP-139 / PAP-72)
//
// Migration `00179` put `FORCE ROW LEVEL SECURITY` on every ESG table, so each
// query MUST run on a connection that has `app.current_org_id` set or it
// collapses to deny-all (PAP-72). Each handler therefore acquires an
...
// the caller was validated against — never a client-supplied value, so the SQL
// org filter and the RLS context can never disagree. By-id repo queries stay
// keyed on `(id, organization_id)` (PAP-136 defense-in-depth), so a
// cross-tenant probe resolves to no row → `404`/no-op even on an RLS-exempt
// pool. `rls.release()` clears the context before the connection returns to
// the pool.
...
/// Map a `sqlx::Error` to a 500 response.
...
async fn get_metric(
    ...
    let out = state
        ...
        .await
        .map_err(internal_error)
        .and_then(|m| m.ok_or((StatusCode::NOT_FOUND, "Metric not found".to_string())))
        .map(Json);
    rls.release().await;
    out
}
...
async fn get_target(
    ...
    let out = state
        ...
        .await
        .map_err(internal_error)
        .and_then(|t| t.ok_or((StatusCode::NOT_FOUND, "Target not found".to_string())))
        .map(Json);
    rls.release().await;
    out
}
...
async fn get_report(
    ...
    let out = state
        ...
        .await
        .map_err(internal_error)
        .and_then(|r| r.ok_or((StatusCode::NOT_FOUND, "Report not found".to_string())))
        .map(Json);
    rls.release().await;
    out
}
...
async fn get_eu_taxonomy_assessment(
    ...
    let out = state
        ...
        .await
        .map_err(internal_error)
        .and_then(|a| a.ok_or((StatusCode::NOT_FOUND, "Assessment not found".to_string())))
        .map(Json);
    rls.release().await;
    out
}
...
async fn get_import_job(
    ...
    let out = state
        ...
        .await
        .map_err(internal_error)
        .and_then(|j| j.ok_or((StatusCode::NOT_FOUND, "Import job not found".to_string())))
        .map(Json);
    rls.release().await;
    out
}
