//! Regression tests for the AML/DSA authz & data-integrity gaps (PAP-43,
//! child of the PAP-35 security review of `routes/aml_dsa.rs`).
//!
...
async fn resolve_content_owner_is_org_scoped(pool: PgPool) {
    ...
    let no_org = repo
        ...
        .await
        .expect("query ok");
    assert_eq!(no_org, None, "org-scoped content needs an org context to resolve");
}
...
async fn resolve_content_owner_unknown_content_is_none(pool: PgPool) {
    ...
    for ct in [ModeratedContentType::Review, ModeratedContentType::Comment] {
        let r = repo
            ...
            .await
            .expect("query ok");
        assert_eq!(r, None, "{ct:?} has no owner table and must not invent an owner");
    }
}
...
async fn file_appeal_rejects_cross_tenant_case(pool: PgPool) {
    ...
    let req = app
        .post(&format!("/api/v1/aml-dsa/moderation/cases/{case_id}/appeal"))
        .bearer(&token)
