...
//!
//! Covers:
//!   - GET /api/v1/accounting/statement-lines/{id}/matches  (list_matches)
...
async fn accounting_matches_statements_happy_path(pool: PgPool) {
    ...
    if let Some(stmt_id) = stmt_id_opt {
        ...
        if let Some(line_id) = line_id_opt {
            ...
            // 3.1 GET /accounting/statement-lines/{id}/matches -> list_matches
            let resp = app
                .execute(
                    session
                        .get(&format!(
                            "/api/v1/accounting/statement-lines/{line_id}/matches"
                        ))
                        .build(),
                )
                .await;
