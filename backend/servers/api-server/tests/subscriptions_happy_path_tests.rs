...
async fn subscriptions_endpoints_happy_path(pool: PgPool) {
    ...
    // 4.3 GET /api/v1/subscriptions/usage/current -> get_current_usage
    let resp = app
        .execute(
            session
                .get("/api/v1/subscriptions/usage/current?metric_type=api_calls")
                .build(),
        )
        .await;
    resp.assert_status(StatusCode::OK);
    ...
    let resp = app
        .execute(
            session
                .post("/api/v1/subscriptions/coupons/redeem")
                .json(&redeem_payload)
                .build(),
        )
        .await;
    ...
    let resp = app
        .execute(
            session
                .delete(&format!("/api/v1/subscriptions/payment-methods/{pm_id}"))
                .build(),
        )
        .await;
    resp.assert_status(StatusCode::NO_CONTENT);
