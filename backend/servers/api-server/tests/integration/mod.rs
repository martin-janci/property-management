...
// document_folder_tests moved to the root-level `tests/document_folder_tests.rs`
// binary (this `integration/` subtree is never declared as `mod integration;` by
// any test bin, so it does not compile — see that file's header note).
