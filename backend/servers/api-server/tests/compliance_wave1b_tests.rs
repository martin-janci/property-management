...
//!
//! Primary assertion: the role gate rejects under-privileged callers with 403.
//! Secondary assertions: authorized callers get 200 with the expected body
//! shape, and idempotent lookups return 404 for unknown IDs where applicable.
...
async fn list_aml_assessments_requires_compliance_role(pool: PgPool) {
    ...
    let req = app
        ...
        .bearer(&token)
        .build();
    assert_eq!(app.execute(req).await.status, StatusCode::FORBIDDEN);
}
...
async fn list_aml_assessments_compliance_user_gets_paginated_response(pool: PgPool) {
    ...
    assert!(body["assessments"].is_array());
...
async fn get_aml_assessment_requires_compliance_role(pool: PgPool) {
    ...
    let req = app
        .get(&format!(
            "/api/v1/aml-dsa/aml/assessments/{}",
            Uuid::new_v4()
        ))
        .bearer(&token)
        .build();
...
async fn get_aml_assessment_returns_404_for_unknown_id(pool: PgPool) {
    ...
    let req = app
        ...
        .tenant(org_id)
        .build();
    assert_eq!(app.execute(req).await.status, StatusCode::NOT_FOUND);
}
...
async fn get_aml_assessment_returns_body_shape_for_own_assessment(pool: PgPool) {
    ...
    let req = app
        .get(&format!(
            "/api/v1/aml-dsa/aml/assessments/{assessment_id}"
        ))
        .bearer(&token)
        .tenant(org_id)
        .build();
...
async fn get_edd_record_requires_compliance_role(pool: PgPool) {
    ...
    let token = mint_token(user_id, None, None);
    let req = app
        .get(&format!("/api/v1/aml-dsa/edd/{}", Uuid::new_v4()))
        .bearer(&token)
        .build();
    assert_eq!(app.execute(req).await.status, StatusCode::FORBIDDEN);
}
...
async fn verify_edd_document_requires_compliance_role(pool: PgPool) {
    ...
    let req = app
        .post(&format!(
            "/api/v1/aml-dsa/edd/{}/documents/{}/verify",
            Uuid::new_v4(),
            Uuid::new_v4()
        ))
        .bearer(&token)
...
async fn verify_edd_document_updates_status(pool: PgPool) {
    ...
    let upload = app
        ...
        .build();
    let upload_resp = app.execute(upload).await;
    assert_eq!(upload_resp.status, StatusCode::OK);
    let doc_id = upload_resp.json_value()["id"].as_str().unwrap().to_string();
    ...
    assert_eq!(resp.status, StatusCode::OK);
    assert_eq!(
        resp.json_value()["verification_status"].as_str().unwrap(),
        "verified"
    );
}
...
async fn add_edd_note_requires_compliance_role(pool: PgPool) {
    ...
    let token = mint_token(user_id, None, None);
    let req = app
        .post(&format!("/api/v1/aml-dsa/edd/{}/notes", Uuid::new_v4()))
        .bearer(&token)
...
async fn add_edd_note_stores_note_and_returns_body(pool: PgPool) {
    ...
    let req = app
        ...
        .bearer(&token)
        .tenant(org_id)
        .json(json!({ "content": "Verified source of funds via bank statement." }))
        .build();
    let resp = app.execute(req).await;
    assert_eq!(resp.status, StatusCode::OK);
    let body = resp.json_value();
...
async fn complete_edd_requires_compliance_role(pool: PgPool) {
    ...
    let token = mint_token(user_id, None, None);
    let req = app
        .post(&format!("/api/v1/aml-dsa/edd/{}/complete", Uuid::new_v4()))
        .bearer(&token)
        .build();
    assert_eq!(app.execute(req).await.status, StatusCode::FORBIDDEN);
}
...
async fn list_pending_edd_returns_own_org_records(pool: PgPool) {
    ...
    for record in arr.as_array().unwrap() {
        ...
        assert_ne!(initiated_by_str, other_user.to_string());
    }
}
...
async fn publish_dsa_report_requires_platform_role(pool: PgPool) {
    ...
    let req = app
        .post(&format!(
            "/api/v1/aml-dsa/dsa/reports/{}/publish",
            Uuid::new_v4()
        ))
        .bearer(&token)
        .build();
...
async fn get_moderation_queue_manager_gets_paginated_cases(pool: PgPool) {
    ...
    let resp = app.execute(req).await;
    assert_eq!(resp.status, StatusCode::OK);
    assert!(resp.json_value()["cases"].is_array());
}
...
async fn get_moderation_stats_returns_stats_shape(pool: PgPool) {
    ...
    assert_eq!(resp.status, StatusCode::OK);
    let body = resp.json_value();
    assert!(body["pending_count"].is_number());
    assert!(body["under_review_count"].is_number());
}
...
async fn get_moderation_case_requires_moderator_role(pool: PgPool) {
    ...
    let req = app
        .get(&format!(
            "/api/v1/aml-dsa/moderation/cases/{}",
            Uuid::new_v4()
        ))
        .bearer(&token)
        .build();
...
async fn assign_moderation_case_requires_moderator_role(pool: PgPool) {
    ...
    let req = app
        .post(&format!(
            "/api/v1/aml-dsa/moderation/cases/{}/assign",
            Uuid::new_v4()
        ))
        .bearer(&token)
        .json(json!({ "moderator_id": Uuid::new_v4() }))
        .build();
    assert_eq!(app.execute(req).await.status, StatusCode::FORBIDDEN);
}
...
async fn decide_appeal_requires_moderator_role(pool: PgPool) {
    ...
    let req = app
        .post(&format!(
            "/api/v1/aml-dsa/moderation/cases/{}/appeal/decide",
            Uuid::new_v4()
        ))
        .bearer(&token)
        .json(json!({ "decision": "upheld", "rationale": "Justified." }))
        .build();
    assert_eq!(app.execute(req).await.status, StatusCode::FORBIDDEN);
}
...
async fn get_audit_logs_requires_super_admin(pool: PgPool) {
    ...
    let req = app.get("/api/v1/compliance/audit-logs").bearer(&token).build();
    assert_eq!(app.execute(req).await.status, StatusCode::FORBIDDEN);
}
...
async fn get_user_audit_logs_requires_super_admin(pool: PgPool) {
    ...
    let req = app
        .get(&format!(
            "/api/v1/compliance/audit-logs/user/{}",
            Uuid::new_v4()
        ))
        .bearer(&token)
        .build();
...
async fn get_user_audit_logs_super_admin_returns_filtered_logs(pool: PgPool) {
    ...
    let resp = app.execute(req).await;
    assert_eq!(resp.status, StatusCode::OK);
    assert!(resp.json_value()["logs"].is_array());
}
...
async fn verify_audit_integrity_super_admin_returns_integrity_report(pool: PgPool) {
    ...
    let resp = app.execute(req).await;
    assert_eq!(resp.status, StatusCode::OK);
    assert!(resp.json_value()["verified"].is_boolean());
}
