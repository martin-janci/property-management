...
async fn market_pricing_endpoints_happy_path(pool: PgPool) {
    ...
    let unit_id = sqlx::query_scalar::<_, Uuid>(
        r#"INSERT INTO units (building_id, unit_number, floor, area_sqm)
           VALUES ($1, '2B', 2, 60.0) RETURNING id"#,
    )
    .bind(building_id)
    .fetch_one(&app.pool)
    .await
    ...
    assert!(regions.as_array().map(|a| !a.is_empty()).unwrap_or(false));
    ...
    if let Some(rid) = rec_id {
        ...
        let reject_payload = json!({ "reason": "too high" });
        let resp = app
            .execute(
                session
    ...
    // 1.5 DELETE /regions/{id} -> delete_region
    let resp = app
        .execute(
            session
    ...
    // ========================================================================
    // 7. DASHBOARD & COMPARABLES
    // ========================================================================

    // 7.1 GET /dashboard -> get_pricing_dashboard
    let resp = app
        .execute(session.get(&format!("{base}/dashboard")).build())
        .await;
    resp.assert_status(StatusCode::OK);

    // 7.2 GET /comparables -> get_comparables
    let resp = app
        .execute(session.get(&format!("{base}/comparables")).build())
        .await;
    resp.assert_status(StatusCode::OK);
}
