...
async fn setup(pool: PgPool, slug: &str) -> Fixture {
    ...
    Fixture { app, token, org_id, building_id, user_id, investor_id, portfolio_id }
}
...
async fn ip_get_investor_succeeds(pool: PgPool) {
    ...
    let resp = f
        ...
        .execute(
            f.app
                .get(&format!(
                    "/api/v1/investor-portal/investors/{}", f.investor_id
                ))
                .bearer(&f.token)
                .header("X-Tenant-ID", &f.org_id.to_string())
                .build(),
...
async fn ip_get_portfolio_succeeds(pool: PgPool) {
    ...
    let resp = f
        ...
        .execute(
            f.app
                .get(&format!(
                    "/api/v1/investor-portal/portfolios/{}", f.portfolio_id
                ))
                .bearer(&f.token)
                .header("X-Tenant-ID", &f.org_id.to_string())
                .build(),
...
async fn ip_list_investor_portfolios_succeeds(pool: PgPool) {
    ...
    let resp = f
        ...
        .await;
    assert_eq!(resp.status, StatusCode::OK, "list investor portfolios: {}", resp.text());
}
...
async fn ip_get_latest_roi_succeeds(pool: PgPool) {
    ...
    let resp = f
        ...
        .await;
    assert_eq!(resp.status, StatusCode::OK, "get latest roi: {}", resp.text());
}
...
async fn ip_list_investor_distributions_succeeds(pool: PgPool) {
    ...
    let resp = f
        ...
        .await;
    assert_eq!(resp.status, StatusCode::OK, "list distributions: {}", resp.text());
}
...
async fn ip_list_investor_capital_calls_succeeds(pool: PgPool) {
    ...
    let resp = f
        ...
        .await;
    assert_eq!(resp.status, StatusCode::OK, "list capital calls: {}", resp.text());
}
...
async fn ip_update_capital_call_succeeds(pool: PgPool) {
    ...
    let cc_id =
        seed_capital_call(&pool, f.org_id, f.portfolio_id, f.investor_id, f.user_id).await;
    let resp = f
        .app
        .execute(
            f.app
...
async fn ip_get_investor_dashboard_succeeds(pool: PgPool) {
    ...
    let resp = f
        ...
        .execute(
            f.app
                .get(&format!(
                    "/api/v1/investor-portal/dashboard/{}", f.investor_id
                ))
                .bearer(&f.token)
                .header("X-Tenant-ID", &f.org_id.to_string())
                .build(),
