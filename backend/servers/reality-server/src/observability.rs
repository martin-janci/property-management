...
impl Default for OtelConfig {
    fn default() -> Self {
        Self {
            service_name: "reality-server".to_string(),
    ...
    fn default() -> Self {
        Self {
            port: std::env::var("METRICS_PORT")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(9091), // Different default port than api-server
...
fn init_metrics(config: &MetricsConfig) -> Option<PrometheusHandle> {
    ...
    metrics::describe_counter!("listings_search_total", "Total number of listing searches");
    metrics::describe_counter!("listings_view_total", "Total number of listing views");
    metrics::describe_counter!("favorites_added_total", "Total number of favorites added");
    metrics::describe_counter!("inquiries_sent_total", "Total number of inquiries sent");
    metrics::describe_counter!("sso_logins_total", "Total number of SSO logins");
