...
//! accept any `Executor`, while the multi-query dashboard helper takes
//! `&mut PgConnection` so it can reborrow across queries. The repository
//! holds no pool — there is no non-RLS code path.
//!
//! ## Example
//!
//! ```rust,ignore
...
impl BudgetRepository {
    ...
    // ===========================================
    // Dashboard Operations (multi-query, RLS-aware)
    // ===========================================
    //
    // This method runs several queries that cannot share a single by-value
    // executor, so it takes `&mut PgConnection` and reborrows it per query.
    // Callers pass `&mut **rls.conn()` so every query runs under the caller's
    // RLS context. The `budgets` / `capital_plans` / `budget_variance_alerts`
    // tables are FORCE-RLS, so a non-RLS connection here is deny-all for
    // own-org traffic.
