...
mod tests {
    ...
    async fn test_create_invoice_with_totals(pool: sqlx::PgPool) {
        ...
        let org_id: uuid::Uuid = sqlx::query("INSERT INTO organizations (name, slug) VALUES ($1, $2) RETURNING id")
            .bind("Test Org")
            .bind("test-org")
            .fetch_one(&pool)
            .await
        ...
        let invoice = repo.create_invoice_rls(&mut *conn, create_data).await.unwrap();
        ...
        // Verify items
        let items = repo.find_invoice_items_rls(&pool, invoice.id).await.unwrap();
        assert_eq!(items.len(), 2);
        
        let consulting = items.iter().find(|i| i.description == "Consulting").unwrap();
        assert_eq!(consulting.vat_rate, Decimal::from(20));
        assert_eq!(consulting.base_amount, Decimal::from_str("1000.00").unwrap());
        assert_eq!(consulting.vat_amount, Decimal::from_str("200.00").unwrap());
        assert_eq!(consulting.total_amount, Decimal::from_str("1200.00").unwrap());
    }
}
