...
const STATUS_COLORS: Record<string, { bg: string; color: string; label: string }> = {
  active: { bg: 'var(--ppt-color-success-light, #d1fae5)', color: 'var(--ppt-color-success-dark, #047857)', label: 'Aktívny' },
  sold: { bg: 'var(--ppt-color-info-light, #dbeafe)', color: 'var(--ppt-color-info-dark, #1e40af)', label: 'Predaný' },
  rented: { bg: 'var(--ppt-color-warning-light, #fef3c7)', color: 'var(--ppt-color-warning-dark, #b45309)', label: 'Prenajatý' },
};
  ...
  return (
    ...
    >
      ...
      <main style={{ flex: 1 }}>
        ...
        >
          <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px 24px', position: 'relative' }}>
            {/* Avatar */}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
              <div>
                ...
                <p style={{ color: 'var(--ppt-fg-secondary)', margin: '0 0 6px', fontSize: '0.9375rem' }}>{profile.bio}</p>
                <p style={{ color: 'var(--ppt-fg-muted, #9ca3af)', fontSize: '0.8125rem', margin: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap' }}>
                {[
                  ...
                  <div key={stat.label} style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--ppt-fg-primary)' }}>{stat.value}</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--ppt-fg-muted, #9ca3af)', whiteSpace: 'nowrap' }}>{stat.label}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {TABS.map((tab) => (
              <button
                ...
                style={{
                  ...
                  background: 'none',
                  border: 'none',
                  borderBottom: activeTab === tab.id ? '2px solid var(--ppt-color-primary, #2563eb)' : '2px solid transparent',
                  color: activeTab === tab.id ? 'var(--ppt-color-primary, #2563eb)' : 'var(--ppt-fg-secondary)',
          ...
          {activeTab === 'listings' && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>
              {MOCK_LISTINGS.map((listing) => (
                ...
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={listing.imageUrl} alt={listing.title} style={{ width: '100%', height: 180, objectFit: 'cover' }} />
                  <div style={{ padding: 16 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                      <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--ppt-fg-primary)', margin: '0 0 4px', lineHeight: 1.3 }}>
                        {listing.title}
                      </h3>
                      <span
                        style={{
                    ...
                    <p style={{ fontSize: '0.875rem', color: 'var(--ppt-fg-secondary)', margin: '0 0 8px' }}>{listing.location}</p>
            ...
            <div style={{ maxWidth: 680 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {MOCK_ACTIVITY.map((entry) => (
                  ...
                  >
                    <span style={{ fontSize: '1.375rem', flexShrink: 0 }}>{ACTIVITY_ICONS[entry.type]}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ color: 'var(--ppt-fg-primary)', fontSize: '0.9375rem' }}>{entry.description}</div>
                      <div style={{ color: 'var(--ppt-fg-muted, #9ca3af)', fontSize: '0.8125rem', marginTop: 3 }}>{entry.timestamp}</div>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    <div style={{ display: 'flex', gap: 2 }}>
                      {Array.from({ length: 5 }).map((_, i) => (
                        <span key={`star-${review.id}-${i}`} style={{ color: i < review.rating ? '#facc15' : '#d1d5db', fontSize: '1rem' }}>★</span>
                      ))}
                    </div>
                  </div>
          ...
          {activeTab === 'settings' && (
            <div style={{ maxWidth: 560 }}>
              <p style={{ color: 'var(--ppt-fg-secondary)' }}>
                ...
                <a href="/account/profile" style={{ color: 'var(--ppt-color-primary, #2563eb)' }}>Môj účet</a>.
              </p>
            </div>
          )}
        </div>
