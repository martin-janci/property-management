...
| Partial — real handler, **no** happy-path test | 1811 | 87.8% |
...
| [ai-automation](./groups/ai-automation.md) | 93 | 20 | 71 | 2 | 21.5% | 97.8% |
| **Total** | **2063** | **127** | **1811** | **125** | **6.2%** | **93.9%** |

Auth-identity leads on test coverage (35%) — unsurprising, since auth got the earliest
and deepest test investment. AI/automation now has its first happy-path coverage (the
20-endpoint `registry.rs` group, BIT-268 wave-4 batch 1); the remaining AI groups are
`RequestPrincipal`-gated and need a `ResolvedTenant`-injecting harness (batch 2).
