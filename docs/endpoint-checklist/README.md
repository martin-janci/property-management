...
## Headline**13.9% of backend endpoints are DONE (implemented *and* test-verified).**

| Status | Count | Share | Meaning |
|--------|------:|------:|---------|
| ✅ `done` | **274** | **13.9%** | Implemented **and** exercised by a passing test |
| 🟡 `partial` | **1642** | 83.2% | Implemented (real DB/logic) but no behaviour-asserting test |
| 🔧 `stub` | **58** | 2.9% | Placeholder handler (501 / mock data) |
| ❌ `missing` | **0** | 0.0% | No handler for a spec/use-case endpoint *(code-first scope — see caveat)* |
| **Total** | **1974** | 100% | Distinct method+path across both servers |

The headline finding: **functionality is broad and largely implemented — the dominant gap is test coverage, not missing features.** 83% of endpoints work but are unproven by tests, so they cannot be counted `done` under the board's "verified by tests" bar.
...
| 04 | Documents, Forms & Signatures | [04-documents-forms-signatures.md](./04-documents-forms-signatures.md) | 61 | 19 | 42 | 0 | 31.1% |
...
| 10 | Integrations & API Ecosystem | [10-integrations.md](./10-integrations.md) | 198 | 16 | 159 | 23 | 8.1% |
| 11 | Financial, Accounting & Reporting | [11-financial-accounting.md](./11-financial-accounting.md) | 227 | 33 | 194 | 0 | 14.5% |
| 12 | Leasing, Listings, Vendors & Buildings | [12-leasing-buildings.md](./12-leasing-buildings.md) | 304 | 19 | 285 | 0 | 6.3% |
...
| | **Total** | | **1974** | **274** | **1642** | **58** | **13.9%** |

> api-server alone: 1883 endpoints, 269 done (14.3%). reality-server: 91 endpoints, 5 done (5.5%).
> Best-covered surface is **Auth/OAuth/MFA (52.8%)** — security-critical paths are the most-tested. Worst is **Platform Admin (1.1%)**.

## Gaps & stubs (58 stub endpoints)
...
Of the 58 stubs: **58 are mounted placeholders returning fake data** (23 migration + 19 regional-compliance + 14 data-residency + 2 OCR). These are the higher risk — they are reachable and return plausible-but-fake responses.
...
Current state: the platform is feature-broad (1974 endpoints, ~97% implemented) but test-thin (only 13.9% test-verified). The path to a higher DONE% is overwhelmingly a **testing** effort, with a smaller stub-completion effort.
...
- **Phase 2 — DONE (BIT-257): deleted the 59 unmounted ROADMAP routers.**
