...
|---|---|---|
| **DONE** — real handler **and** a passing happy-path test | **175** | **8.5%** |
| Partial — real handler, **no** happy-path test | 1763 | 85.5% |
...
- **Test-verified DONE = 8.5%.** This is the strict number the ticket defines: an
  endpoint only counts as done if a test exercises its success path. By that bar the
  platform is ~8.5% done.
...
so they cannot promote an endpoint to `done`. Closing the 8.5% → high-90s gap is
...
| **Total** | **2063** | **175** | **1763** | **125** | **8.5%** | **93.9%** |
...
(93.9%) have real handlers. Security posture is the most-tested dimension. But only 8.5%
