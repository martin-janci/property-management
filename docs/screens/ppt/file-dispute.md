...
- 2026-06-25 — agent: verify-80-2-dispute-filing-flow-to-done. Coverage "verify and finish to done" pass on story 80-2. VERIFIED both documented gaps are already shipped on `dev`: (1) the localized `disputes.draft*` keys (5 keys) are present in ALL 6 message bundles (en/sk/cs/de/pl/hu) — confirmed via grep + the `draftAutosaveI18n.test.ts` regression test (#1360/#1364) that locks key-presence and `{{time}}`/`{{count}}` placeholders per locale; the earlier "English-default `t(key, defaultValue)` fast-follow" is therefore CLOSED, not outstanding. (2) The 5-step wizard redesign remains a redesign TARGET (`redesignStatus: in-progress`), NOT an MVP coverage gap: the MVP single-page filing form is `buildStatus: shipped` / `apiStatus: complete` with all ACs (AC-1 type radio-cards, AC-2 EvidenceUploader, AC-3 validation+counter, AC-4 draft auto-save, AC-5 submit contract) met and tested. No source change required — reconciled stale screen-map Notes that still described the i18n localization as a deferred fast-follow. Verify: `pnpm -F @ppt/web typecheck` + `biome check` clean; `vitest run src/features/disputes` green (incl. `draftAutosaveI18n.test.ts`). buildStatus/redesignStatus/apiStatus unchanged (shipped / in-progress / complete) — the 5-step wizard redesign is intentionally left in-progress.
