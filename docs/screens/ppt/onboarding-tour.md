...
- 2026-07-01 — agent: verify-first pass on Story 10B.6. Screen-map already present and accurate (endpoints match `@ppt/sitemap` operationIds; both related screens exist). Confirmed onboarding is a *deliberately unwired* feature (`ppt-web/src/features/unwired-features.ts`, board decision PAP-55 — keep code, do not route), so route wiring is intentionally out of scope. Closed the genuine remaining gap: added `features/onboarding/types.test.ts` (18 unit tests) covering the pure helpers `calculateTourProgress`, `getCurrentStepIndex`, `getTourDisplayStatus`, `formatVideoDuration` — previously zero coverage. No behaviour change; buildStatus/apiStatus unchanged.
