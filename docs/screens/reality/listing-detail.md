...
- SEO structured data: `ListingDetailContent` emits an inline `<script type="application/ld+json">` (JSON-LD) built from user-supplied listing fields. Because JSON-LD sits in an HTML script context, the payload MUST be serialized with `serializeForScript` from `frontend/apps/reality-web/src/lib/serialize-script.ts` — **not** bare `JSON.stringify` (PR #2600, `ListingDetailContent.test.tsx`). The helper escapes `<`, `>`, `&` and U+2028/U+2029 to `\uXXXX` so a `</script>` (or Unicode line/paragraph separator) inside any listing string can't break out of the script element (stored-XSS sink), while keeping the JSON-LD byte-for-byte valid for crawlers. Any future inline `<script>` payload on this screen (or the shared `[locale]/layout.tsx`) must route through the same primitive.

## Agent Log

<!-- newest entries on top -->

- 2026-08-01 — agent: reconciled screen-map with PR #2600 (reality-web inline-script XSS hardening). PR routed the listing JSON-LD (`ListingDetailContent`'s `<script type="application/ld+json">`, built from user-supplied listing data) through the new shared `serializeForScript` helper (`src/lib/serialize-script.ts`) instead of bare `JSON.stringify`, closing a `</script>` / U+2028–U+2029 breakout (stored-XSS) sink; regression pinned in `ListingDetailContent.test.tsx`. Same PR hardened the shared `[locale]/layout.tsx` ORGANIZATION_LD + per-request tenant bootstrap and `env.js` (documented on `reality/home`, the layout's canonical screen). Added the SEO-structured-data note under Notes > Specific. Frontmatter unchanged: reality-web `buildStatus` stays `shipped` and `apiStatus` stays `partial` — security-hardening of an existing SEO surface, no UI/route/endpoint change. `/screens validate` green.
