 ...
 * nested `ListingSummary` object.  The fields below mirror that shape exactly so the Compose UI
 * can display favorites cards without a second listing-detail request.
 *
 ...
 * - `title`, `current_price`, `currency`, `city`, `property_type`, `transaction_type`,
 *   `photo_url`, `status`, `price_changed`, `price_change_percentage`,
 *   `price_alert_enabled` — flat listing summary embedded by the server.
 * - `listing` — always `null` for this endpoint; kept for backward compat with
 *   any code that still checks `entry.listing`.
 */
@Serializable
 ...
 *
 * The server always returns HTTP 200 with this payload — it never returns 404
 * for an un-favorited listing.  The KMP client must inspect `isFavorited`
 * rather than treating any 2xx as `true`.
 */
@Serializable
