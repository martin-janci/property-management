...
final class AuthManager {
    ...
    /// Service used to wipe persisted navigation state on logout.
    /// Optional so unit tests don't need to construct one.
    private let restorationService: NavigationStateRestorationService?

    /// Currently outstanding SSO CSRF nonce (generated by ``beginSsoFlow()``
    /// and validated by ``consumeSsoState(_:)``). Cleared after a single use
    /// regardless of outcome.
    private var pendingSsoState: String?

    // MARK: - Initialization

    init(restorationService: NavigationStateRestorationService? = nil) {
    ...
    func logout() {
        ...
        // Wipe any persisted navigation state so protected stacks don't
        // outlive the session on disk (NavigationStateRestorationService
        // documents this contract on its `clear()` method).
        restorationService?.clear()
    }

    // MARK: - SSO CSRF nonce

    /// Mint a fresh SSO CSRF nonce and persist it for the duration of the
    /// browser flow. Callers MUST include the returned value as the `state`
    /// query parameter when opening the SSO authorize URL — the server
    /// echoes it back to the deep-link callback so we can detect tampering.
    @discardableResult
    func beginSsoFlow() -> String {
        let nonce = Self.makeSsoState()
        pendingSsoState = nonce
        return nonce
    }

    /// Consume the pending SSO nonce.
    ///
    /// Returns `true` iff `received` matches the value most recently produced
    /// by ``beginSsoFlow()`` (and a nonce is in fact outstanding). The stored
    /// value is cleared after a single check, regardless of outcome, so a
    /// replay or a probe with a missing/empty `state` is rejected.
    func consumeSsoState(_ received: String?) -> Bool {
        defer { pendingSsoState = nil }
        guard let expected = pendingSsoState, !expected.isEmpty,
              let received = received, received == expected else {
            return false
        }
        return true
    }

    private static func makeSsoState() -> String {
        // 32 random bytes → 64-char hex string. Suitable as an OAuth state.
        var bytes = [UInt8](repeating: 0, count: 32)
        let status = SecRandomCopyBytes(kSecRandomDefault, bytes.count, &bytes)
        if status == errSecSuccess {
            return bytes.map { String(format: "%02x", $0) }.joined()
        }
        // Fallback (should not happen on iOS): UUID-based, lower entropy
        // but still unique enough to detect tampering for development.
