...
## Decisions logged 2026-09-26 (Phase 1.6 — pm-scrum-master + pm-security)

- **NEW (2026-09-26, pm-scrum-master):** Close out the current sprint (Epic 6/7A/8A/10A + 10B/80) and cut a new one now that its epics are story-complete at the `development_status` level. The stale summary block in `sprint-status.yaml` should be reconciled first (see `pm-scrum-master-reconcile-stale-epic-counts` action). Owner: pm-scrum-master / PO.
- **NEW (2026-09-26, pm-scrum-master):** Prioritize / fund the cloud-build unblock (issue #2652) — it is now blocking two high-severity security fixes (#2944/#2945) that have failed twice on the same class of error, and 7/8 open backlog items are structurally unclaimable in the cloud runner. Owner: pm-devops.
- **NEW (2026-09-26, pm-security):** Whether to block the next `dev`→`main` release cut until #2944/#2945 IDOR fixes actually merge (not just draft). The tracking issues currently show `closed` on GitHub while the fix PRs (#2976/#2977) remain `open`/`draft` with `mergeable_state: unstable` — the vulnerabilities are still live on `dev`. Owner: pm-security / release manager.
- **NEW (2026-09-26, pm-security):** Whether issue #2652 needs to be split or reclassified — its body describes a mobile-native/KMP `dl.google.com` egress 403, but it is cited as the swagger-ui/api-server blocker for the #2944/#2945 retries. Wrong root-cause tracking delays the real fix. Owner: pm-devops / infra.
