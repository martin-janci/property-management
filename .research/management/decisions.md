...
- **NEW (2026-07-16, pm-security):** Hotfix vs release train for the `create_invitation` cross-tenant IDOR (`backend/servers/reality-server/src/routes/agencies.rs:373` — no `check_agency_membership()`, any authenticated portal user mints 7-day invitation tokens for any agency). Given active exploitability, does this warrant a `hotfix/` branch off `main` per `git-workflow.md`, or bundle into the normal `dev` → `main` release cadence? Owner: release-manager / pm-security.
- **NEW (2026-07-16, pm-security):** Should `list_members` (`agencies.rs` ~345) require agency membership (PII/emails exposed) or remain a public directory endpoint by design? Currently no `RequestPrincipal` extractor. Owner: product-owner.
- **NEW (2026-07-16, pm-scrum-master):** Does `portal_webhooks.rs` (3 edits in 72h + 2 open follow-ups #2358/#2360) warrant a short design note/ADR + code freeze before more feature PRs land piecemeal? Owner: pm-tech-lead.
- **NEW (2026-07-16, pm-scrum-master):** Fold the `epics:` summary block in `sprint-status.yaml` into the `coverage.json`-driven view, or keep maintaining both? Story-level truth diverges from epic-level rollup. Owner: pm-scrum-master.
