...
## Decisions logged 2026-07-31 (Phase 1.6 — pm-scrum-master + pm-frontend)

- **NEW (2026-07-31, pm-frontend):** Add a repo-wide lint/codemod for untrusted-to-union casts and SSR string interpolation into `<script>` without escaping — three same-window findings (#2596 AML decision cast in ppt-web, #2600 tenant JSON XSS in reality-web, #2603 viewsource cast in reality-web) show this is a systemic pattern gap, not isolated bugs. Owner: pm-frontend / pm-tech-lead to pick the mechanism (Biome custom rule vs codemod sweep).
- **NEW (2026-07-31, pm-frontend):** Frontend verify-gate hygiene — pnpm test failures need to be first-class CI signals on dev push, not just verify-gate warnings. #2602 fixed pre-existing failures that had been accumulating for at least one window before being caught. Owner: pm-devops to decide whether frontend.yml gains an on-push test job or verify-gate output escalates to a routine signal.
- **NEW (2026-07-31, pm-scrum-master):** 84-1 direct-to-S3 wiring is now unblocked (dependency #2573 was closed by PR #2597 this window). Recommend explicit implementer claim for gap-84-1 next dispatcher round with a scoped brief (api-client binding + UploadDocument integration + regression test) — the story has been partial for weeks and MVP close needs it plus 84-2 to reach 49/49. Owner: pm-frontend.
