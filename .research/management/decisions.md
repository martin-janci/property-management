...
- **NEW (2026-07-06, pm-security):** Whether to treat the 34-site `TenantExtractor::role.is_manager()` anti-pattern in announcements/documents/notifications/templates (same JWT-role-vs-DB-role class just fixed by #2120 in `outages.rs`) as a release blocker for Epic 6/7A — the anti-pattern silently 403's real managers on core mutations in production while integration tests pass. Owner: pm-security / pm-backend.
- **NEW (2026-07-06, pm-security):** Whether to close or rebase draft PR #1797 (`fix/backend-authz-ocr-and-rental-pii`, 13 days open) given its fix content appears already present in the working tree — risks silent reintroduction of the anonymous-OCR-upload / rental-guest-PII gap on a merge/rebase. Owner: pm-scrum-master.
- **NEW (2026-07-06, pm-security):** Whether to gate 10a-1/10a-2/10a-3 promotion on the still-open OAuth security test suite (#481/#487) this sprint — items unchanged since the 2026-05-27 run. Owner: pm-qa / pm-tech-lead.
