...
- **NEW (2026-06-25, pm-security):** Decide whether unauthenticated `/api/v1/ai/ocr/*` endpoints should be disabled (router-level 404/403) until auth is wired, or if adding the extractor is sufficient given the current 501-stub behavior — owner: pm-security + rust-backend.
- **NEW (2026-06-25, pm-security):** Are Epic 10A stories (10a-1, 10a-3) hard-blocked from staging deploy until issue #481 (refresh-token revocation bypass) is fixed, given RFC 9700 compliance is a stated requirement? — owner: pm-tech-lead.
- **NEW (2026-06-25, pm-scrum-master):** Formal deferred/fix decision for test-hardening batch #480 (JWT in WS logs) and #481 (OAuth revoked-token reuse) — open 31 days, severity=high — owner: pm-security.
- **NEW (2026-06-25, pm-scrum-master):** ACC (Accounting) workstream sprint scope and story-owner assignments before draft PRs (#1821/#1820/#1805/#1803/#1818/#1811) are promoted from draft — owner: pm-scrum-master + pm-backend.
- **NEW (2026-06-25, pm-scrum-master):** Decouple Epic 6 announcement story promotion from ppt/announcements screen redesign sign-off — ship at current impl fidelity, flip `buildStatus=shipped`, track redesign separately — owner: pm-frontend.
