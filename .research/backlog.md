# Backlog of vectors
<sub>Last regenerated: 2026-07-06 10:19 UTC by routine</sub>
...
| 3 | bug | `bug-ppt-web-core-api-client-unconfigured` | ppt-web axios stack sends unauthenticated requests (sentiment, predictive-maintenance) | ready | 2026-07-06 | [plan](plans/bug-ppt-web-core-api-client-unconfigured.md) |
...
| 2 | bug | `bug-ppt-web-core-perf-metrics-listener-leak` | usePerformanceMetrics never removes load/visibilitychange listeners | open | 2026-07-06 |  |
...
| 2 | bug | `code-review-api-core-vote-partial-cmp-panic` | vote.rs:1765 calculate_question_result() uses partial_cmp().unwrap() on f64 — NaN/Inf weights panic /votes/{id}/result | done | 2026-06-16 |  |
| 2 | bug | `code-review-mobile-rn-report-fault-fake-submit` | ReportFaultScreen.tsx handleSubmit() fakes API call with setTimeout(1500) — fault reports never reach backend (App.tsx | ready | 2026-06-16 | [plan](plans/code-review-mobile-rn-report-fault-fake-submit.md) |
| 2 | bug | `code-review-reality-web-realtor-mgmt-untranslated` | Reality-web RealtorManagement.tsx hardcoded English strings — agency flow not localized to sk/cs/de | ready | 2026-06-15 | [plan](plans/code-review-reality-web-realtor-mgmt-untranslated.md) |
| 2 | bug | `code-review-reality-web-share-comparison-404` | Reality-web ComparisonUrlHandler hits non-existent /api/listings/${id} — every shared comparison URL 404s | ready | 2026-06-14 | [plan](plans/code-review-reality-web-share-comparison-404.md) |
| 2 | bug | `code-review-reality-web-listing-page-ssr-crash` | Reality-web listing detail SSR crashes on partial 200 body — JSON-LD build deref of undefined fields | ready | 2026-06-14 | [plan](plans/code-review-reality-web-listing-page-ssr-crash.md) |
...
| 2 | refactor | `refactor-platform-admin-rs-module-split` | platform_admin.rs (2,762 LOC) — explicit module-split into routes/platform_admin/{tenants,features,billing,audit,mod}. | done | 2026-06-06 |  |
...
| 1 | bug | `code-review-mobile-rn-screens-mock-data` | Mobile RN production screens (Buildings/Meters/Leases/PersonMonths/Notifications/Threads/Forms) render hardcoded MOCK_ | open | 2026-07-06 |  |
...
| 1 | refactor | `refactor-churn-hotspot-backend-crates-db-src-repositories-rental-oauth-rs` | churn hotspot: backend/crates/db/src/repositories/rental/oauth.rs | open | 2026-07-06 |  |
| 1 | refactor | `refactor-churn-hotspot-backend-crates-db-src-models-multi-currency-rs` | churn hotspot: backend/crates/db/src/models/multi_currency.rs | open | 2026-07-06 |  |
| 1 | refactor | `refactor-churn-hotspot-backend-crates-db-src-repositories-rental-bookings-rs` | churn hotspot: backend/crates/db/src/repositories/rental/bookings.rs | open | 2026-07-06 |  |
| 1 | refactor | `refactor-stalled-review-pr-1797` | stalled review: PR #1797 auth on OCR endpoints + rental guest PII gate | open | 2026-07-06 |  |
| 1 | refactor | `refactor-stalled-review-pr-1812` | stalled review: PR #1812 reality_portal repository split | open | 2026-07-06 |  |
| 1 | bug | `bug-ppt-web-core-usewebsocket-late-handler` | useWebSocket loses first subscription when handler resolves after mount | open | 2026-07-06 |  |
...
| 1 | test-gap | `test-gap-screen-map-drift-pr-1100-ppt` | Screen-map drift: PR #1100 modified ppt-web App.tsx (FileDisputePageRoute extraction) without screen-doc update | open | 2026-07-05 |  |
...
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1288-webhook-rls` | Webhook handlers RLS migration (PR #1288, PAP-170) shipped without a new regression test for repo-layer methods | open | 2026-07-05 |  |
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1287-rls-llm-sessions` | AI llm/sessions + integrations sync + subscriptions RLS migration (PR #1287, PAP-169) shipped without a new regression | open | 2026-07-05 |  |
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1289-api-ecosystem` | api_ecosystem.rs RLS migration (PR #1289, PAP-167) — 162-line handler rework shipped without a regression test for the | open | 2026-07-05 |  |
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1292-mfa-rls` | mfa.rs RLS migration (PR #1292, PAP-168) shipped without a regression test; also landed broken and was hotfixed in PR  | open | 2026-07-05 |  |
...
| 1 | bug | `code-review-mobile-rn-deeplink-init-unhandled` | useDeepLinkRouting.ts:27-36 — initialize() re-runs on onNavigate identity change + void promise with no .catch → dupli | open | 2026-07-05 |  |
...
| 1 | triage | `triage-issue-848` | Issue #848 (no labels, OPEN): Code review: Epics 78+134 — Vendor portal stubs & Predictive maintenance gaps (origin/de | done | 2026-06-01 |  |
...
| 1 | triage | `triage-issue-867` | Issue #867 (no labels, OPEN): Tech debt: api-server main.rs duplicates lib.rs::create_router — routers diverge silentl | done | 2026-06-01 |  |
...
| 0 | refactor | `churn-hotspot-backend-crates-db-src-repositories-subscription-rs` | Churn hotspot: 2856 lines changed in backend/crates/db/src/repositories/subscription.rs (window 2026-06-10 03:05Z→18:3 | dropped | 2026-07-05 |  |
...
| 0 | triage | `triage-issue-1151` | Issue #1151 (no labels, OPEN): Research dispatcher: claimable buffer is stale — true claimable work = 0 despite metric | dropped | 2026-07-05 |  |
...
| 0 | triage | `triage-issue-951` | Issue #951 (no labels, OPEN): Deploy blocker: api-server requires ESIGN_TOKEN_SECRET + ESIGN_WEBHOOK_SECRET not inject | dropped | 2026-07-05 |  |
| 0 | dx | `closed-not-merged-pr-1274` | PR #1274 (cargo-minor-patch group, /backend, 9 updates) closed unmerged — superseded by #1313 after auto-rebase fix la | dropped | 2026-07-05 |  |
...
| 0 | refactor | `churn-hotspot-backend-servers-api-server-src-routes-api-ecosystem-rs` | Churn hotspot: backend/servers/api-server/src/routes/api_ecosystem.rs (+106/−27 in PR #1293 PAP-171; second touch in 2 | dropped | 2026-07-05 |  |
...
| 0 | refactor | `churn-hotspot-backend-servers-api-server-src-routes-forms-rs` | Churn hotspot: backend/servers/api-server/src/routes/forms.rs touched 2x since 2026-06-12 (window 2026-06-12→2026-06-1 | dropped | 2026-07-05 |  |
| 0 | refactor | `churn-hotspot-backend-servers-api-server-tests-reserve-funds-cross-org-idor-tests-rs` | Churn hotspot: backend/servers/api-server/tests/reserve_funds_cross_org_idor_tests.rs touched 2x since 2026-06-12 (win | dropped | 2026-07-05 |  |
| 0 | refactor | `churn-hotspot-backend-crates-db-tests-form-rls-repo-tests-rs` | Churn hotspot: backend/crates/db/tests/form_rls_repo_tests.rs touched 2x since 2026-06-12 (window 2026-06-12→2026-06-1 | dropped | 2026-07-05 |  |
...
| 0 | dx | `dx-stalled-review-pr-1179` | PR #1179 (docs(epics) catalog backfill for 37 mounted-but-undocumented backend modules) — stalled at 7d, no reviewDeci | dropped | 2026-07-05 |  |
