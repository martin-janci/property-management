# Backlog of vectors
<sub>Last regenerated: 2026-07-31 14:26 UTC by routine</sub>
...
| 3 | security | `code-review-reality-server-ssrf-ipv4-mapped-ipv6` | backend/servers/reality-server/src/util/url_validator.rs:146-165 — the `Host::Ipv6` branch of `validate_fetch_url` only  | ready | 2026-07-31 | [plan](plans/code-review-reality-server-ssrf-ipv4-mapped-ipv6.md) |
...
| 3 | dx | `dx-api-validation-drift-gate-never-runs-on-dev` | SDK drift gate is effectively unenforced — api-validation.yml only fires on docs/api/**, so committed @ppt/api-client dr | done | 2026-07-28 |  |
...
| 3 | security | `code-review-api-handlers-community-cross-tenant-idor` | SECURITY: community.rs 5 write handlers (create_post/add_reaction/create_comment/rsvp_event/create_inquiry) accept cross | done | 2026-07-23 | [plan](plans/code-review-api-handlers-community-cross-tenant-idor.md) |
...
| 3 | bug | `code-review-mobile-rn-report-fault-fake-submit` | ReportFaultScreen.tsx handleSubmit() fakes API call with setTimeout(1500) — fault reports never reach backend (App.tsx:1 | dropped | 2026-06-16 | [plan](plans/code-review-mobile-rn-report-fault-fake-submit.md) |
...
| 2 | test-gap | `screen-map-drift-pr-2600-reality` | Screen-map drift: PR #2600 touched frontend routes without docs/screens/ update | open | 2026-07-31 |  |
| 2 | test-gap | `hotfix-no-test-pr-2601` | hotfix-no-test: PR #2601 shipped fix with no test-file changes | open | 2026-07-31 |  |
| 2 | test-gap | `hotfix-no-test-pr-2592` | hotfix-no-test: PR #2592 shipped fix with no test-file changes | open | 2026-07-31 |  |
| 2 | bug | `code-review-api-handlers-install-empty-cred-default` | backend/servers/api-server/src/routes/integrations/install.rs:1061 and :1065 (fn sync_booking, line 1007) — Booking.com  | open | 2026-07-31 |  |
| 2 | bug | `code-review-reality-server-agent-review-body-unbounded` | backend/servers/reality-server/src/routes/agent_reviews.rs:190-266 — the public `create_review` handler validates only ` | open | 2026-07-31 |  |
| 2 | bug | `code-review-reality-server-ss-alert-watermark-swallow` | backend/servers/reality-server/src/services/saved_search_alerts.rs:237 — the saved-search enqueue worker advances the pe | open | 2026-07-31 |  |
| 2 | bug | `code-review-reality-server-redirect-backslash-bypass` | backend/servers/reality-server/src/routes/sso.rs:148-151 — `check_redirect_uri_allowed` treats any value starting with ` | open | 2026-07-31 |  |
| 2 | bug | `code-review-api-core-resolved-rs-leaks-db-error` | SECURITY-LITE: layout/resolved.rs err500 handler leaks raw sqlx/serde error text on public GET /layout/resolved/{screen} | done | 2026-07-30 |  |
| 2 | security | `code-review-reality-web-inline-tenant-json-xss` | SECURITY: reality-web layout.tsx inlines tenant-config JSON into <script> without </script>/U+2028/U+2029 escaping — HTM | done | 2026-07-28 |  |
...
| 2 | refactor | `refactor-churn-hotspot-api-server-reports-2026-07-27` | Churn hotspot: backend/servers/api-server/src/routes/reports.rs — 3329 lines this window, runs_seen=3 (repeated instabil | done | 2026-07-27 |  |
...
| 2 | test-gap | `screen-map-drift-pr-2431-reality` | Screen-map drift: PR #2431 touched reality-web/src/app/api/layout-revalidate/route.ts without updating docs/screens/real | done | 2026-07-21 |  |
| 2 | bug | `code-review-ppt-web-ui-propinput-json-coerce` | TenantSectionEditor PropInput silently JSON.parse-coerces every string prop on blur — override payload corrupted ("true" | done | 2026-07-21 |  |
| 2 | bug | `code-review-ppt-web-ui-savedirty-stale-closure` | DashboardCustomizePage 'changed since sent' check is tautological — concurrent edits during in-flight save are silently  | done | 2026-07-21 |  |
| 2 | bug | `code-review-ppt-web-ui-actionqueue-mock-shipped` | Dashboard useActionQueue queryFn returns generateMockData — production users see fabricated action items; approve/reject | done | 2026-07-21 |  |
| 2 | security | `code-review-api-core-scheduler-units-target-cross-tenant` | scheduler.rs units/buildings target queries lack organization_id AND-scope — fan-out can leak across tenants if create-a | done | 2026-07-20 |  |
...
| 1 | refactor | `refactor-churn-hotspot-integrations-booking-mod-rs-2026-07-27` | Churn hotspot: backend/crates/integrations/src/booking/mod.rs — 3185 lines this window (post-PR-#2176 tail) | done | 2026-07-31 |  |
| 1 | refactor | `churn-hotspot-backend/servers/api-server/src/routes/reports/schedule_cadence.rs` | Churn hotspot: backend/servers/api-server/src/routes/reports/schedule_cadence.rs | open | 2026-07-31 |  |
| 1 | refactor | `churn-hotspot-backend/servers/api-server/src/routes/voice_webhooks.rs` | Churn hotspot: backend/servers/api-server/src/routes/voice_webhooks.rs | open | 2026-07-31 |  |
| 1 | bug | `code-review-api-core-push-fanout-reqwest-expect` | backend/servers/api-server/src/services/push_fanout.rs:280 and :773 — the FCM and APNs transport adapters build their re | open | 2026-07-31 |  |
| 1 | bug | `code-review-api-handlers-auth-2fa-flag-fail-false` | backend/servers/api-server/src/routes/auth.rs:~2324 (GET /me) and :~2416 (profile update response) compute the response  | open | 2026-07-31 |  |
...
| 1 | refactor | `refactor-churn-hotspot-api-server-layout-tenant-2026-07-30` | Churn hotspot: backend/servers/api-server/src/routes/layout/tenant.rs — 262 lines this window (PR #2478) | open | 2026-07-30 |  |
...
| 1 | bug | `code-review-api-core-admin-rs-swallowed-serialize` | layout/admin.rs (+ tenant.rs) mutation handlers end with unwrap_or_default() serialize — a failed serialize returns 200  | done | 2026-07-30 |  |
| 1 | bug | `code-review-api-core-scheduler-rs-silent-target-err` | scheduler.rs silently swallows DB errors on notification target lookups at 3 sites — failed dispatches show as empty tar | done | 2026-07-30 |  |
| 1 | refactor | `code-review-reality-web-viewsource-untrusted-cast` | reality-web listingAnalytics.ts casts untrusted ?source= query param straight to ViewSource union — pollutes listing.vie | done | 2026-07-28 |  |
| 1 | dx | `dx-stale-todo-security-comments-faults-critical-notifications` | Stale TODO(security) headers in faults.rs / critical_notifications.rs describe a hardcoded-false gate that no longer exi | done | 2026-07-28 |  |
...
| 1 | bug | `code-review-api-core-scheduler-target-ids-silent-parse` | scheduler.rs get_announcement_target_users() silently swallows target_ids JSON parse errors — malformed payload publishe | done | 2026-07-20 |  |
| 1 | refactor | `refactor-churn-hotspot-mobile-package-json-2026-07-20` | Churn hotspot: frontend/apps/mobile/package.json — 5 touches this window (Expo/expo-notifications/expo-config-plugins de | done | 2026-07-20 |  |
...
| 1 | dx | `dx-closed-not-merged-pr-2385` | PR #2385 (dependabot: dtolnay/rust-toolchain 1.94.1 → 1.100.0) closed unmerged — likely superseded by unrelated toolchai | dropped | 2026-07-20 |  |
...
| 1 | refactor | `refactor-churn-hotspots-en-json-2026-07-16` | Churn hotspot: frontend/apps/ppt-web/messages/en.json — frontend/apps/ppt-web/messages/en.json: +3926 lines this run (ru | done | 2026-07-16 |  |
| 1 | refactor | `refactor-churn-hotspots-sitemap-json-2026-07-16` | Churn hotspot: frontend/packages/sitemap/src/json/sitemap.json — frontend/packages/sitemap/src/json/sitemap.json: +3727  | done | 2026-07-16 |  |
...
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1287-rls-llm-sessions` | AI llm/sessions + integrations sync + subscriptions RLS migration (PR #1287, PAP-169) shipped without a new regression t | dropped | 2026-07-05 |  |
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1289-api-ecosystem` | api_ecosystem.rs RLS migration (PR #1289, PAP-167) — 162-line handler rework shipped without a regression test for the p | dropped | 2026-07-05 |  |
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1292-mfa-rls` | mfa.rs RLS migration (PR #1292, PAP-168) shipped without a regression test; also landed broken and was hotfixed in PR #1 | dropped | 2026-07-05 |  |
...
| 1 | bug | `code-review-mobile-rn-screens-mock-data` | Mobile RN production screens (Buildings/Meters/Leases/PersonMonths/Notifications/Threads/Forms) render hardcoded MOCK_*  | done | 2026-07-05 |  |
| 1 | bug | `code-review-mobile-rn-deeplink-init-unhandled` | useDeepLinkRouting.ts:27-36 — initialize() re-runs on onNavigate identity change + void promise with no .catch → duplica | dropped | 2026-07-05 |  |
...
| 0 | triage | `triage-issue-1151` | Issue #1151 (no labels, OPEN): Research dispatcher: claimable buffer is stale — true claimable work = 0 despite metric=5 | dropped | 2026-07-05 |  |
...
| 0 | triage | `triage-issue-951` | Issue #951 (no labels, OPEN): Deploy blocker: api-server requires ESIGN_TOKEN_SECRET + ESIGN_WEBHOOK_SECRET not injected | dropped | 2026-07-05 |  |
| 0 | dx | `closed-not-merged-pr-1274` | PR #1274 (cargo-minor-patch group, /backend, 9 updates) closed unmerged — superseded by #1313 after auto-rebase fix land | dropped | 2026-07-05 |  |
...
| 0 | refactor | `churn-hotspot-backend-servers-api-server-tests-reserve-funds-cross-org-idor-tests-rs` | Churn hotspot: backend/servers/api-server/tests/reserve_funds_cross_org_idor_tests.rs touched 2x since 2026-06-12 (windo | dropped | 2026-07-05 |  |
...
| 0 | dx | `dx-stalled-review-pr-1179` | PR #1179 (docs(epics) catalog backfill for 37 mounted-but-undocumented backend modules) — stalled at 7d, no reviewDecisi | dropped | 2026-07-05 |  |
