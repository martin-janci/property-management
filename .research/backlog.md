# Backlog of vectors
<sub>Last regenerated: 2026-08-11 20:15 UTC by routine</sub>
...
| 2 | bug | `code-review-api-handlers-webhook-ssrf-loopback-failopen` | backend/servers/api-server/src/routes/integrations/webhook.rs:201-203 (create_webhook_subscription) ... | open | 2026-08-11 |  |
| 2 | bug | `code-review-api-handlers-residency-compliance-failopen` | in the data-residency dashboard handler, the top-level `compliance_status` field is computed as `if ... | open | 2026-08-11 |  |
| 2 | bug | `code-review-mobile-native-kmp-create-listing-stub` | the production nav graph wires the CreateListing route's submit callback to a hardwired stub: `Creat... | open | 2026-08-11 |  |
| 2 | bug | `code-review-ppt-web-core-auth-401-refresh-unwired` | the axios response interceptor has a reactive 401 hook: `if (error.response?.status === 401 && onUna... | open | 2026-08-11 |  |
| 2 | bug | `code-review-ppt-web-ui-facilities-booking-silent-errors` | handleApprove awaits the approve-booking mutation inside try/catch, and on failure the catch body is... | open | 2026-08-11 |  |
| 2 | bug | `code-review-ppt-web-ui-registry-silent-mutation-errors` | handlePetDelete confirms via window.confirm then `await deletePetMutation.mutateAsync(id)`; the catc... | open | 2026-08-11 |  |
| 2 | bug | `code-review-reality-server-agent-review-self-review` | create_review (POST /api/v1/realtors/{id}/reviews) lets an authenticated portal principal post a rat... | open | 2026-08-11 |  |
| 2 | bug | `code-review-mobile-native-kmp-cancellation-swallowed` | every suspend network method wraps the Ktor call in `catch (e: Exception) { Result.failure(e) }`. Be... | open | 2026-08-11 |  |
| 2 | bug | `code-review-mobile-native-kmp-httpclient-no-timeout` | the single shared Ktor HttpClient installs ContentNegotiation, Logging(NONE), and a defaultRequest c... | open | 2026-08-11 |  |
| 2 | bug | `code-review-mobile-native-kmp-create-listing-notimpl-stub` | the CreateListing destination wires `onSubmit = { _ -> Result.failure(NotImplementedError("Wire to l... | open | 2026-08-11 |  |
| 2 | bug | `code-review-mobile-native-kmp-mylistings-truncates-100-no-paging` | the MyListings route loads the realtor's own listings once via `portalListingsRepository.listMyListi... | open | 2026-08-11 |  |
| 2 | bug | `code-review-mobile-rn-widget-apirequest-swallows-errors` | the private `apiRequest<T>()` used by every widget fetch (fetchNotificationsData, fetchUpcomingMeeti... | open | 2026-08-11 |  |
| 2 | bug | `code-review-ppt-web-core-api-onunauthorized-unwired` | the hand-rolled axios client is configured with ONLY the token getter: `configureApiClient({ getToke... | open | 2026-08-11 |  |
| 2 | bug | `code-review-ppt-web-ui-content-mod-unvalidated-prompt-cast` | handleTakeAction reads a free-text moderation decision via window.prompt() and casts it directly to ... | open | 2026-08-11 |  |
| 2 | bug | `code-review-reality-web-listings-page-no-error-state` | `const { data, isLoading } = useListings(filters, currentPage);` destructures ONLY data + isLoading.... | open | 2026-08-11 |  |
| 2 | bug | `code-review-reality-web-layoutsections-dup-key` | both the placeholder branch and the real-component branch render each row with `key={section.type}` ... | open | 2026-08-11 |  |
...
| 1 | refactor | `churn-hotspot-backend/servers/api-server/tests/suites/oauth_integration_tests.rs` | Churn hotspot: backend/servers/api-server/tests/suites/oauth_integration_tests.rs | open | 2026-08-11 |  |
| 1 | dx | `stalled-review-pr-2559` | open 13d with no reviewDecision: feat(acc): PAY by square payment QR endpoint (UC-ACC-05.8) — author... | open | 2026-08-11 |  |
| 1 | dx | `stalled-review-pr-2558` | open 13d with no reviewDecision: feat(acc): invoice PDF render endpoint (UC-ACC-05.9) — author marti... | open | 2026-08-11 |  |
| 1 | dx | `stalled-review-pr-2555` | open 13d with no reviewDecision: feat(acc): wire sent/cancelled invoice lifecycle (UC-ACC-05.17) — a... | open | 2026-08-11 |  |
| 1 | bug | `code-review-api-core-listings-detail-swallows-db-err` | in `get_listing` (the tenant-scoped GET /api/v1/listings/{id} detail handler, listings.rs:316-345), ... | open | 2026-08-11 |  |
| 1 | bug | `code-review-api-handlers-voice-oauth-link-failopen` | when `VoiceOAuthManager::from_env()` reports the platform is not configured (`!oauth_manager.has_pla... | open | 2026-08-11 |  |
| 1 | bug | `code-review-api-handlers-residency-chain-valid-failopen` | in `list_audit_logs_impl`, the audit-chain tamper-evidence flag is read as `let chain_valid = verifi... | open | 2026-08-11 |  |
| 1 | test-gap | `code-review-mobile-native-kmp-ssoservice-no-unit-test` | it owns the session-token lifecycle (validateAndLogin, restoreSession, login, register, requestPassw... | open | 2026-08-11 |  |
| 1 | completeness | `code-review-ppt-web-ui-registry-review-approve-only` | handlePetReview hardcodes `data: { approve: true }` and is guarded only by `window.confirm('Approve ... | open | 2026-08-11 |  |
| 1 | bug | `code-review-reality-server-agent-review-body-unbounded` | CreateReviewRequest { rating: i32, body: Option<String> }. In create_review the rating is validated ... | open | 2026-08-11 |  |
| 1 | test-gap | `code-review-mobile-native-kmp-ssoservice-no-test` | createMobileToken, loginWithPassword, register, requestPasswordReset/confirm, getSession, refreshSes... | open | 2026-08-11 |  |
| 1 | bug | `code-review-mobile-rn-widget-storage-unguarded-jsonparse` | []` and :347-352 `getAllCachedData()` does `return stored ? JSON.parse(stored) : {}` — both parse As... | open | 2026-08-11 |  |
| 1 | bug | `code-review-ppt-web-core-ws-token-snapshot` | WebSocketWrapper passes `auth={{ accessToken: getAccessToken(), isAuthenticated }}`, i.e. a point-in... | open | 2026-08-11 |  |
| 1 | quality | `code-review-ppt-web-core-errorhandler-i18n-gap` | the entire ERROR_MESSAGES map plus DEFAULT_ERROR are hardcoded English title/message strings ('Authe... | open | 2026-08-11 |  |
| 1 | bug | `code-review-ppt-web-ui-protectedroute-i18n-gap` | ProtectedRoute renders user-facing English string literals with no i18n: the loading state hardcodes... | open | 2026-08-11 |  |
| 1 | bug | `code-review-ppt-web-ui-offlineindicator-i18n-gap` | the offline banner text "You are offline. Some features may be unavailable." and the reconnected ban... | open | 2026-08-11 |  |
| 1 | bug | `code-review-ppt-web-ui-confirmdialog-i18n-gap` | the confirm button renders `{isLoading ? 'Processing...' : confirmLabel}`. The in-progress label "Pr... | open | 2026-08-11 |  |
| 1 | bug | `code-review-ppt-web-ui-content-mod-window-location-nav` | handleViewContent navigates with `window.location.href = `/compliance/moderation/cases/${caseId}`` i... | open | 2026-08-11 |  |
| 1 | bug | `code-review-reality-web-maint-fallback-i18n` | the `building_disabled` defense-in-depth branch renders hardcoded English: `<h1>Service temporarily ... | open | 2026-08-11 |  |
...
| 0 | refactor | `churn-hotspot-frontend/packages/sitemap/src/json/sitemap.json` | Churn hotspot: frontend/packages/sitemap/src/json/sitemap.json | open | 2026-08-11 |  |
| 0 | refactor | `churn-hotspot-frontend/apps/ppt-web/messages/en.json` | Churn hotspot: frontend/apps/ppt-web/messages/en.json | open | 2026-08-11 |  |
