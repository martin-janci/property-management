# Backlog of vectors
<sub>Last regenerated: 2026-06-18 08:22 UTC by routine</sub>
...
| 3 | bug | `bug-dev-baseline-ci-test-job-red-1332-recurrence` | Backend `test` CI job red on dev baseline (#1332 recurrence) — blocks auto-merge across every open PR | open | 2026-06-18 |  |
| 3 | dx | `dx-dispatcher-self-approve-gate-blocking-merges` | Dispatcher reviewer cannot self-approve own implementer PRs — verdict=approve lands as comment, auto-merge never fires (buffer wedge) | open | 2026-06-18 |  |
...
| 2 | test-gap | `test-gap-hotfix-no-test-pr-1545-reality-web-prerender` | Reality-web /accounting force-dynamic prerender hotfix (PR #1545) shipped without a prerender regression test | open | 2026-06-18 |  |
| 2 | test-gap | `test-gap-hotfix-no-test-pr-1539-oauth-public-client` | OAuth public-client client_secret reject hotfix (PR #1539) shipped without a regression test | open | 2026-06-18 |  |
...
| 0 | triage | `triage-issue-951` | Issue #951 (no labels, OPEN): Deploy blocker: api-server requires ESIGN_TOKEN_SECRET + ESIGN_WEBHOOK_SECRET not injected by deploy-server (staging/prod) | dropped | 2026-06-18 |  |
