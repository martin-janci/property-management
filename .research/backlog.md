# Backlog of vectors
<sub>Last regenerated: 2026-06-18 22:28 UTC by routine</sub>
...
| 2 | test-gap | `test-gap-hotfix-no-test-pr-1582-mobile-folder-wiring` | Mobile folder-tree wiring (PR #1582) shipped without a real wiring test — pin-test asserts only the constant value | open | 2026-06-18 |  |
...
| 1 | refactor | `repeated-churn-backend-crates-integrations-src-booking-rs` | Repeated churn: booking.rs — runs_seen=1→2 (6 commits this run, OTA Booking.com integration) | open | 2026-06-18 |  |
| 1 | refactor | `repeated-churn-backend-crates-db-src-repositories-document-rs` | Repeated churn: document.rs — runs_seen=1→2 (8 commits this run after 2026-06-10 hotspot) | open | 2026-06-18 |  |
| 1 | refactor | `churn-hotspot-backend-crates-db-src-repositories-rental-rs` | Churn hotspot: rental.rs — 8 commits this run (enum cast fixes across BIT-118 / GUEST / NULL-fee booking write path) | open | 2026-06-18 |  |
...
| 1 | security | `security-pkce-oauth-authcode-pr-908-closed` | PR #908 (fix(security): require PKCE on OAuth authorization-code flow, closes #823) was closed unmerged — verify whether PKCE enforcement still pendin | done | 2026-06-03 |  |
...
| 0 | triage | `triage-issue-951` | Issue #951 (no labels, OPEN): Deploy blocker: api-server requires ESIGN_TOKEN_SECRET + ESIGN_WEBHOOK_SECRET not injected by deploy-server (staging/pro | dropped | 2026-06-18 |  |
