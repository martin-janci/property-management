# Backlog of vectors
<sub>Last regenerated: 2026-07-12 16:34 UTC by routine</sub>
...
| 3 | security | `security-code-review-api-handlers-suspended-enum-oracle` | Suspended-account enum oracle: 401 ACCOUNT_SUSPENDED returned before password verified | ready | 2026-07-12 | [plan](plans/security-code-review-api-handlers-suspended-enum-oracle.md) |
| 3 | security | `security-code-review-reality-server-ssrf-ipv4-mapped-ipv6` | SSRF: validate_fetch_url accepts ::ffff:169.254.169.254 (cloud metadata bypass) | ready | 2026-07-12 | [plan](plans/security-code-review-reality-server-ssrf-ipv4-mapped-ipv6.md) |
...
| 3 | bug | `code-review-mobile-rn-report-fault-fake-submit` | ReportFaultScreen.tsx handleSubmit() fakes API call with setTimeout(1500) — fault reports never reach backend  | dropped | 2026-06-16 | [plan](plans/code-review-mobile-rn-report-fault-fake-submit.md) |
...
| 2 | refactor | `refactor-churn-hotspot-backend-servers-api-server-src-routes-auth-rs` | Reduce churn instability in api-server routes/auth.rs — 3rd run at top-N churn | open | 2026-07-12 |  |
| 2 | security | `security-code-review-api-handlers-raw-db-error-leak` | 437 handler arms return raw sqlx e.to_string() as client-facing error body — info disclosure | open | 2026-07-12 |  |
...
| 1 | bug | `bug-code-review-mobile-native-kmp-no-http-timeout` | KMP HttpClient missing HttpTimeout plugin — hanging spinners on stalled connections | open | 2026-07-12 |  |
| 1 | bug | `bug-code-review-ppt-web-ui-moderation-prompt-unvalidated` | ContentModerationPage prompt() values coerced to literal unions without validation | open | 2026-07-12 |  |
| 1 | refactor | `refactor-churn-hotspot-backend-crates-db-src-repositories-llm-document-rs` | Watch llm_document.rs churn — RAG + KMP path-param work | open | 2026-07-12 |  |
| 1 | refactor | `refactor-churn-hotspot-backend-crates-integrations-src-booking-mod-rs` | Watch booking OTA client churn — 2 test-coverage passes landed back-to-back | open | 2026-07-12 |  |
| 1 | refactor | `refactor-code-review-ppt-web-ui-sessionspage-unsafe-cast` | SessionsPage.tsx: double 'as unknown as' cast at API boundary — strip cast, use generated type | open | 2026-07-12 |  |
...
| 1 | bug | `code-review-mobile-native-kmp-deeplink-token-not-url-decoded` | DeepLinkRouter skips URL-decoding while Android Uri.getQueryParameter decodes — SSO tokens diverge per platfor | dropped | 2026-07-05 |  |
...
| 1 | bug | `code-review-mobile-rn-deeplink-init-unhandled` | useDeepLinkRouting.ts:27-36 — initialize() re-runs on onNavigate identity change + void promise with no .catch | dropped | 2026-07-05 |  |
| 1 | bug | `code-review-mobile-rn-screens-mock-data` | Mobile RN production screens (Buildings/Meters/Leases/PersonMonths/Notifications/Threads/Forms) render hardcod | done | 2026-07-05 |  |
...
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1287-rls-llm-sessions` | AI llm/sessions + integrations sync + subscriptions RLS migration (PR #1287, PAP-169) shipped without a new re | dropped | 2026-07-05 |  |
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1288-webhook-rls` | Webhook handlers RLS migration (PR #1288, PAP-170) shipped without a new regression test for repo-layer method | dropped | 2026-07-05 |  |
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1289-api-ecosystem` | api_ecosystem.rs RLS migration (PR #1289, PAP-167) — 162-line handler rework shipped without a regression test | dropped | 2026-07-05 |  |
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1292-mfa-rls` | mfa.rs RLS migration (PR #1292, PAP-168) shipped without a regression test; also landed broken and was hotfixe | dropped | 2026-07-05 |  |
...
| 1 | test-gap | `test-gap-screen-map-drift-pr-1100-ppt` | Screen-map drift: PR #1100 modified ppt-web App.tsx (FileDisputePageRoute extraction) without screen-doc updat | dropped | 2026-07-05 |  |
| 1 | test-gap | `test-gap-screen-map-drift-pr-922-ppt` | Screen-map drift: PR #922 modified ppt-web App.tsx (dev-review rounds 1-5 fixes) without a docs/screens/ppt up | done | 2026-07-05 |  |
...
| 0 | refactor | `churn-hotspot-backend-crates-db-src-repositories-document-rs` | Churn hotspot: 2940 lines changed in backend/crates/db/src/repositories/document.rs (window 2026-06-10 03:05Z→ | dropped | 2026-07-05 |  |
...
| 0 | refactor | `churn-hotspot-backend-crates-db-src-repositories-subscription-rs` | Churn hotspot: 2856 lines changed in backend/crates/db/src/repositories/subscription.rs (window 2026-06-10 03: | dropped | 2026-07-05 |  |
| 0 | refactor | `churn-hotspot-backend-crates-db-tests-form-rls-repo-tests-rs` | Churn hotspot: backend/crates/db/tests/form_rls_repo_tests.rs touched 2x since 2026-06-12 (window 2026-06-12→2 | dropped | 2026-07-05 |  |
...
| 0 | refactor | `churn-hotspot-backend-servers-api-server-src-routes-aml-dsa-rs` | Churn hotspot: 2691 lines changed in backend/servers/api-server/src/routes/aml_dsa.rs (window 2026-06-10 03:05 | dropped | 2026-07-05 |  |
| 0 | refactor | `churn-hotspot-backend-servers-api-server-src-routes-api-ecosystem-rs` | Churn hotspot: backend/servers/api-server/src/routes/api_ecosystem.rs (+106/−27 in PR #1293 PAP-171; second to | dropped | 2026-07-05 |  |
...
| 0 | refactor | `churn-hotspot-backend-servers-api-server-src-routes-forms-rs` | Churn hotspot: backend/servers/api-server/src/routes/forms.rs touched 2x since 2026-06-12 (window 2026-06-12→2 | dropped | 2026-07-05 |  |
...
| 0 | refactor | `churn-hotspot-backend-servers-api-server-tests-reserve-funds-cross-org-idor-tests-rs` | Churn hotspot: backend/servers/api-server/tests/reserve_funds_cross_org_idor_tests.rs touched 2x since 2026-06 | dropped | 2026-07-05 |  |
...
| 0 | dx | `closed-not-merged-pr-1274` | PR #1274 (cargo-minor-patch group, /backend, 9 updates) closed unmerged — superseded by #1313 after auto-rebas | dropped | 2026-07-05 |  |
...
| 0 | dx | `dx-stalled-review-pr-1179` | PR #1179 (docs(epics) catalog backfill for 37 mounted-but-undocumented backend modules) — stalled at 7d, no re | dropped | 2026-07-05 |  |
| 0 | dx | `dx-stalled-review-pr-988` | Stalled review: PR #988 (Epic: reusable Playwright E2E framework + sitemap FlowRunner) open 10d, no reviewDeci | dropped | 2026-07-05 |  |
...
| 0 | triage | `triage-issue-1151` | Issue #1151 (no labels, OPEN): Research dispatcher: claimable buffer is stale — true claimable work = 0 despit | dropped | 2026-07-05 |  |
| 0 | triage | `triage-issue-1331` | Issue #1331 (no labels, OPEN): Backend `test` job red/hanging on dev base — blocks the entire backend merge pi | dropped | 2026-07-05 |  |
...
| 0 | triage | `triage-issue-951` | Issue #951 (no labels, OPEN): Deploy blocker: api-server requires ESIGN_TOKEN_SECRET + ESIGN_WEBHOOK_SECRET no | dropped | 2026-07-05 |  |
