# Backlog of vectors
<sub>Last regenerated: 2026-06-27 22:22 UTC by routine</sub>
...
| 3 | dx | `revert-delegations-pr-1690-process` | Process gap: gap-sweep re-admitted board-retired delegations frontend (PR #1690 reverted by #1713) | open | 2026-06-27 |  |
| 3 | bug | `code-review-mobile-rn-report-fault-fake-submit` | ReportFaultScreen.tsx handleSubmit() fakes API call with setTimeout(1500) — fault reports never reach backend (App.tsx | ready | 2026-06-16 | [plan](plans/code-review-mobile-rn-report-fault-fake-submit.md) |
...
| 2 | bug | `code-review-mobile-rn-deeplink-init-unhandled` | useDeepLinkRouting.ts:27-36 — initialize() re-runs on onNavigate identity change + void promise with no .catch → dupli | open | 2026-06-16 |  |
...
| 2 | bug | `bug-report-schedule-update-no-sql` | ReportSchedule.update_schedule stores cron in `time` workaround; documented UPDATE never runs (missing cron_expression | done | 2026-05-30 |  |
...
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1287-rls-llm-sessions` | AI llm/sessions + integrations sync + subscriptions RLS migration (PR #1287, PAP-169) shipped without a new regression | open | 2026-06-27 |  |
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1289-api-ecosystem` | api_ecosystem.rs RLS migration (PR #1289, PAP-167) — 162-line handler rework shipped without a regression test for the | open | 2026-06-27 |  |
| 1 | test-gap | `test-gap-hotfix-no-test-pr-1292-mfa-rls` | mfa.rs RLS migration (PR #1292, PAP-168) shipped without a regression test; also landed broken and was hotfixed in PR  | open | 2026-06-27 |  |
| 1 | refactor | `churn-hotspot-backend-crates-db-src-repositories-rental-rs` | Churn hotspot: rental.rs — 2933 lines added in 11-day window (sustained feature/coverage churn) | open | 2026-06-27 |  |
| 1 | refactor | `churn-hotspot-backend-servers-api-server-src-routes-migration-rs` | Churn hotspot: migration.rs — 2921 lines changed (+2338/-583) in BIT-256 tenant-migration landing | open | 2026-06-27 |  |
| 1 | refactor | `churn-hotspot-backend-crates-integrations-src-booking-mod-rs` | Churn hotspot: booking/mod.rs — 2818 lines added (file split from booking.rs into directory) | open | 2026-06-27 |  |
| 1 | refactor | `code-review-ppt-web-core-aichat-session-staletime` | useAiChatSession query missing staleTime — indefinite stale reads on session metadata | open | 2026-06-27 |  |
| 1 | refactor | `code-review-ppt-web-core-aichat-feedback-invalidate` | useMessageFeedback mutation lacks cache invalidation — escalated-messages cache stays stale | open | 2026-06-27 |  |
...
| 1 | dx | `dx-stalled-review-pr-1179` | PR #1179 (docs(epics) catalog backfill for 37 mounted-but-undocumented backend modules) — stalled at 7d, no reviewDeci | open | 2026-06-16 |  |
...
| 1 | security | `security-pkce-oauth-authcode-pr-908-closed` | PR #908 (fix(security): require PKCE on OAuth authorization-code flow, closes #823) was closed unmerged — verify wheth | done | 2026-06-03 |  |
...
| 1 | triage | `triage-issue-859` | Issue #859 (no labels, OPEN): sqlx 0.9 breaks runtime decode of Postgres enum columns into Rust String (SELECT * reads | done | 2026-06-01 |  |
...
| 0 | refactor | `churn-hotspot-backend-servers-api-server-tests-reserve-funds-cross-org-idor-tests-rs` | Churn hotspot: backend/servers/api-server/tests/reserve_funds_cross_org_idor_tests.rs touched 2x since 2026-06-12 (win | dropped | 2026-06-13 |  |
...
| 0 | dx | `closed-not-merged-pr-1274` | PR #1274 (cargo-minor-patch group, /backend, 9 updates) closed unmerged — superseded by #1313 after auto-rebase fix la | dropped | 2026-06-12 |  |
...
| 0 | refactor | `churn-hotspot-backend-servers-api-server-src-routes-api-ecosystem-rs` | Churn hotspot: backend/servers/api-server/src/routes/api_ecosystem.rs (+106/−27 in PR #1293 PAP-171; second touch in 2 | dropped | 2026-06-12 |  |
...
| 0 | refactor | `churn-hotspot-backend-crates-db-src-repositories-subscription-rs` | Churn hotspot: 2856 lines changed in backend/crates/db/src/repositories/subscription.rs (window 2026-06-10 03:05Z→18:3 | dropped | 2026-06-10 |  |
...
| 0 | triage | `triage-issue-1151` | Issue #1151 (no labels, OPEN): Research dispatcher: claimable buffer is stale — true claimable work = 0 despite metric | dropped | 2026-06-07 |  |
...
| 0 | triage | `triage-issue-951` | Issue #951 (no labels, OPEN): Deploy blocker: api-server requires ESIGN_TOKEN_SECRET + ESIGN_WEBHOOK_SECRET not inject | dropped | 2026-06-03 |  |
