...
| 2 | bug | `code-review-api-core-announcement-roles-legacy-membership` | api-core: the announcement target-resolution helper's `"roles"` branch fans out via `SELECT DISTINCT | open | 2026-08-21 |  |
...
| 2 | bug | `code-review-ppt-web-core-api-401-onunauthorized-unwired` | ppt-web-core: the hand-rolled axios response interceptor has an unauthorized branch `if (error.respo | open | 2026-08-21 |  |
| 2 | bug | `code-review-ppt-web-ui-moderation-prompt-cast-unvalidated` | ppt-web-ui: handleTakeAction reads free-text `window.prompt(t('moderation.prompts.actionType'), 'app | open | 2026-08-21 |  |
| 2 | bug | `code-review-ppt-web-ui-dsa-download-no-onsuccess` | ppt-web-ui: handleDownload calls `downloadPdf.mutate(reportId, { onError: ... })` with ONLY an onErr | open | 2026-08-21 |  |
| 2 | bug | `code-review-api-core-voice-facility-contact-fabricated-success` | api-core: `action_book_facility` returns `VoiceActionResult { success: true, .. }` while speaking 'F | open | 2026-08-21 |  |
| 2 | bug | `code-review-api-core-syndication-stub-never-called` | api-core: `SyndicationService::process_syndication_job` (public API) has zero callers in the tree. ` | open | 2026-08-21 |  |
| 2 | bug | `code-review-api-handlers-list-limit-unclamped` | api-handlers: Multiple list handlers under backend/servers/api-server/src/routes/ take a user-contro | open | 2026-08-21 |  |
| 2 | bug | `code-review-api-handlers-financial-error-swallow` | api-handlers: backend/servers/api-server/src/routes/financial.rs:561-571 (unit-ledger handler) match | open | 2026-08-21 |  |
| 2 | bug | `code-review-ppt-web-ui-ai-chat-ocr-fetch-no-auth` | ppt-web-ui: frontend/apps/ppt-web/src/features/ai-chat/hooks/useAiChat.ts:31-46 (apiFetch) and front | open | 2026-08-21 |  |
| 2 | bug | `code-review-ppt-web-ui-ai-chat-delete-swallows-err` | ppt-web-ui: frontend/apps/ppt-web/src/features/ai-chat/hooks/useAiChat.ts:131-142 (useDeleteSession) | open | 2026-08-21 |  |
| 2 | bug | `code-review-ppt-web-ui-moderation-prompt-cast` | ppt-web-ui: handleTakeAction reads action_type from window.prompt (default | open | 2026-08-21 |  |
...
| 2 | bug | `code-review-reality-web-sso-callback-error-passthrough` | reality-web: the SSO callback pulls `error` and `error_description` off the query string | open | 2026-08-21 |  |
...
| 1 | triage | `closed-not-merged-pr-2750` | Dependabot PR #2750 closed unmerged (superseded by newer version bump) | open | 2026-08-21 |  |
| 1 | triage | `closed-not-merged-pr-2751` | Dependabot PR #2751 closed unmerged (superseded by newer version bump) | open | 2026-08-21 |  |
| 1 | bug | `code-review-api-core-delay-to-duration-mul-overflow` | api-core: `DelayConfig::to_duration()` computes `Duration::from_secs(self.duration * 60|3600|86400)` | open | 2026-08-21 |  |
| 1 | bug | `code-review-mobile-native-kmp-formatprice-minor-unit-contract` | mobile-native-kmp: FormatUtils.formatPrice(price: Long, currency) documents '@param price The price  | open | 2026-08-21 |  |
| 1 | bug | `code-review-mobile-native-kmp-ssoinitiation-callback-unencoded` | mobile-native-kmp: `fun buildUrl(state: String): String = "$PM_APP_SCHEME://sso?callback=$CALLBACK&s | open | 2026-08-21 |  |
| 1 | bug | `code-review-ppt-web-core-refresh-queue-never-invoked` | ppt-web-core: `refreshToken` implements a careful concurrency-safe silent-refresh: it de-duplicates  | open | 2026-08-21 |  |
| 1 | bug | `code-review-ppt-web-core-api-default-export-stale-singleton` | ppt-web-core: `export default getApiClient();` is evaluated at module-load time. `getApiClient()` la | open | 2026-08-21 |  |
| 1 | bug | `code-review-ppt-web-ui-compliance-hardcoded-i18n` | ppt-web-ui: frontend/apps/ppt-web/src/features/compliance/pages/AmlDashboardPage.tsx hardcodes Engli | open | 2026-08-21 |  |
| 1 | bug | `code-review-ppt-web-ui-ai-chat-querykey-paging` | ppt-web-ui: useAiChatSessions(limit=50, offset=0) (L49-60), useAiChatMessages(sessionId, limit=100 | open | 2026-08-21 |  |
| 1 | dx | `code-review-ppt-web-ui-dsa-reports-i18n` | ppt-web-ui: user-facing status strings are hardcoded English literals in | open | 2026-08-21 |  |
| 1 | dx | `code-review-ppt-web-ui-registry-confirm-i18n` | ppt-web-ui: destructive/approve confirmations use native window.confirm() wit | open | 2026-08-21 |  |
| 1 | bug | `code-review-reality-web-listinggrid-empty-i18n` | reality-web: the PRIMARY listing-search-results grid has zero next-intl usage (no useTranslatio | open | 2026-08-21 |  |
