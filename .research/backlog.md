# Backlog of vectors
<sub>Last regenerated: 2026-08-03 17:00 UTC by routine</sub>
...
| 2 | test-gap | `screen-map-drift-pr-2636-reality` | docs/screens/reality/agency-import + agency-inquiries out of sync with PR #2636 i18n rewrite | open | 2026-08-03 |  |
| 2 | bug | `code-review-ppt-web-core-rentals-auth-nonnull-mutation` | ppt-web rentals mutations dereference `auth!` non-null — mid-session token loss throws uncaught TypeError instead of handled auth error | open | 2026-08-03 |  |
...
| 2 | bug | `code-review-ppt-web-core-route-error-boundary-gap` | ppt-web has no route-outlet ErrorBoundary — a single stale-chunk lazy() rejection unmounts the entire application shell | open | 2026-08-03 |  |
...
| 2 | bug | `code-review-mobile-rn-thread-send-silent-fail` | mobile: ThreadDetailScreen.handleSend send-message mutation has onSuccess only, no onError — failed sends are silent | open | 2026-08-03 |  |
| 2 | bug | `code-review-mobile-rn-offline-sync-false-complete` | mobile: useOfflineSupport.processQueue reports isComplete:true after a head-of-line-blocked (halted) sync cycle | open | 2026-08-03 |  |
| 2 | bug | `code-review-ppt-web-core-api-retry-nonidempotent` | ppt-web axios retry interceptor retries non-idempotent POST/PUT on 5xx / network errors — risk of duplicate server-side writes | open | 2026-08-03 |  |
| 2 | bug | `code-review-ppt-web-core-api-token-provider-unset` | ppt-web getApiClient() sends requests with no Authorization header — configureApiClient is never called, so the token provider stays undefined | open | 2026-08-03 |  |
...
| 1 | bug | `code-review-mobile-rn-push-debug-console-log` | mobile: usePushNotifications — debug console.log left on the device-token register/unregister production path | open | 2026-08-03 |  |
| 1 | bug | `code-review-mobile-rn-hardcoded-empty-error-i18n` | mobile: empty/error/loading state strings hardcoded in English across ~18 screens despite react-i18next being wired | open | 2026-08-03 |  |
