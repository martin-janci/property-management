# Backlog of vectors
<sub>Last regenerated: 2026-08-08 22:07 UTC by routine</sub>
...
| 2 | bug | `code-review-api-core-delay-duration-overflow-before-cap` | `DelayConfig::to_duration()` multiplies the config-supplied `duration: u64` by a | open | 2026-08-08 |  |
| 2 | bug | `code-review-api-handlers-reports-unwrap-or-default-silent-zero` | four report handlers swallow repository DB errors with `.unwrap_or_default()` an | open | 2026-08-08 |  |
| 2 | bug | `code-review-mobile-native-kmp-create-listing-submit-stub` | the Screen.CreateListing composable wires CreateListingScreen(onSubmit = { _ ->  | open | 2026-08-08 |  |
| 2 | bug | `code-review-ppt-web-core-errorhandler-network-misclassify` | parseApiError() runs isNetworkError(error) as its FIRST branch, before the struc | open | 2026-08-08 |  |
| 2 | bug | `code-review-sso-rs-negcache-transient-fail` | introspect_pm_token() caches active=false for the full 60s TTL on ANY non-succes | open | 2026-08-08 |  |
| 2 | bug | `code-review-sso-rs-client-no-timeout-pool` | each SSO OAuth helper constructs a fresh `reqwest::Client::new()` per invocation | open | 2026-08-08 |  |
...
| 1 | refactor | `churn-hotspot-backend-servers-api-server-src-services-scheduler-mod-rs` | 482 lines churned across merged PR range #2703-#2723 (this run) | open | 2026-08-08 |  |
| 1 | refactor | `churn-hotspot-backend-servers-api-server-src-services-actions-api_call-rs` | 345 lines churned across merged PR range #2703-#2723 (this run) | open | 2026-08-08 |  |
| 1 | refactor | `churn-hotspot-backend-servers-api-server-src-routes-layout-admin-rs` | 330 lines churned across merged PR range #2703-#2723 (this run) | open | 2026-08-08 |  |
| 1 | bug | `code-review-api-core-quiet-hours-db-error-fail-open` | the user quiet-hours lookup swallows repository errors: `self.granular_repo.get_ | open | 2026-08-08 |  |
| 1 | refactor | `code-review-mobile-native-kmp-apiclient-dead-code-healthcheck-bug` | class ApiClient is never instantiated anywhere in production code. grep '\bApiCl | open | 2026-08-08 |  |
| 1 | bug | `code-review-ppt-web-core-usewebsocket-handler-dep-gap` | the subscribe effect early-returns when `!eventType || !handlerRef.current` and  | open | 2026-08-08 |  |
