# Backlog of vectors
<sub>Last regenerated: 2026-08-08 16:19 UTC by routine</sub>
...
| 2 | bug | `code-review-api-core-accounting-line-multimatch` | Accounting payment auto-matcher: single line produces duplicate suggestions when multiple invoices share a Variable Symbol | open | 2026-08-08 |  |
| 2 | bug | `code-review-api-core-delay-duration-overflow-before-cap` | DelayConfig::to_duration multiplies unchecked before 7d cap — overflow bypasses guard in release | open | 2026-08-08 |  |
| 2 | bug | `code-review-api-handlers-reports-unwrap-or-default-silent-zero` | reports/mod.rs: 4 report handlers .unwrap_or_default() on secondary queries, serve 200 with silently-zeroed data | open | 2026-08-08 |  |
| 2 | bug | `code-review-mobile-native-kmp-create-listing-submit-stub` | mobile-native realtor CreateListing screen: submit hardcoded to Result.failure(NotImplementedError) in production nav | open | 2026-08-08 |  |
| 2 | bug | `code-review-sso-rs-negcache-transient-fail` | reality-server SSO introspect_pm_token: negatively caches active=false for 60s on 5xx/network error (transient PM outage → user logged out for a minute after PM recovers) | open | 2026-08-08 |  |
| 2 | bug | `code-review-sso-rs-client-no-timeout-pool` | reality-server SSO helpers construct reqwest::Client::new() per call — no timeout, no pool/TLS reuse (hang risk if PM stalls) | open | 2026-08-08 |  |
...
| 1 | bug | `code-review-api-core-accounting-vs-only-amount-ignored` | Accounting matcher accepts VS-only match (0.8 score) with amount wildly mismatched | open | 2026-08-08 |  |
| 1 | test-gap | `code-review-api-core-accounting-no-tests` | accounting.rs (339 lines, payment auto-matching) has no unit tests at all | open | 2026-08-08 |  |
| 1 | bug | `code-review-api-core-quiet-hours-db-error-fail-open` | notification_pipeline: DB error on quiet-hours lookup silently .ok()-swallows, delivers push during user's quiet window | open | 2026-08-08 |  |
| 1 | refactor | `code-review-mobile-native-kmp-apiclient-dead-code-healthcheck-bug` | mobile-native ApiClient.kt is dead code (~100 lines) with latent healthCheck() .toString() bug | open | 2026-08-08 |  |
| 1 | refactor | `churn-hotspot-backend-servers-reality-server-src-routes-inquiries-rs` | Churn hotspot: reality-server/src/routes/inquiries.rs (3 changes this run) | open | 2026-08-08 |  |
| 1 | refactor | `churn-hotspot-backend-servers-reality-server-src-state-rs` | Churn hotspot: reality-server/src/state.rs (3 changes this run) | open | 2026-08-08 |  |
